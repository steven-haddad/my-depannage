	...
	<div class="an_productattributes-dropdown-menu">
		<ul>
			...
			<li role="option" data-value="{$k|intval}"{if !$v.availableForOrder} class="an_productattributes-select-sold-out"{/if}>
				...
				<span class="an_productattributes-select-sale" style="background: {$config.background_sold_out}; color: {$config.color_sold_out};">{$v.prices.discount_percentage}
				{/if}
				{if $v.prices.discount_type == 'amount'}
				<span class="an_productattributes-select-sale" style="background: {$config.background_sale}; color: {$config.color_sale};">Save {$v.prices.discount_amount}
				{/if}				
				</span>
				{/if}				
			</li>
