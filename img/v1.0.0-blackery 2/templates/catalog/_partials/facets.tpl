    ...
    {foreach from=$facets item="facet"}
      {if $facet.displayed}
        <section class="facet clearfix">
          ...
          {/foreach}
          <div class="title">
            <h5 class="h5 facet-title">{$facet.label}</h1>
            ...
            {block name='facet_item_other'}
              <ul id="facet_{$_expand_id}" {if $facetedsearch != 'standard'} class="collapse {/if} {if !$_collapse or $facetedsearch == 'turnon'} in{/if}">
                {foreach from=$facet.filters key=filter_key item="filter"}
                  {if $filter.displayed}
                    <li {if isset($filter.properties.color)}class="color-list"{/if}>
                        ...
                        {if $facet.multipleSelectionAllowed}
                          <span class="custom-checkbox">
                            ...
                            {if isset($filter.properties.color)}
                              ...
                              <span class="color {if ($filter.properties.color == '#ffffff') }color-white{/if}" style="background-color:{$filter.properties.color}"></span>
